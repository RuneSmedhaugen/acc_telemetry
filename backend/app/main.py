...
def verify_api_key(request: Request):
    ...
    row = con.execute("SELECT id FROM api_keys WHERE api_key = ?", (key,)).fetchone()
    if not row:
...
def create_api_key():
    last_id = con.execute("SELECT MAX(id) FROM api_keys").fetchone()[0]
    new_id = (last_id or 0) + 1
    key = generate_api_key()
    con.execute("INSERT INTO api_keys VALUES (?, ?, ?)", (new_id, key, datetime.now()))
    con.commit()
    return {"api_key": key}
...
def reset_key(request: Request):
    ...
    con.execute("DELETE FROM api_keys WHERE api_key = ?", (key_to_delete,))
    con.commit()
...
def get_sessions(request: Request):
    verify_api_key(request)
    rows = con.execute("SELECT * FROM sessions ORDER BY session_date DESC").fetchall()
    return {
        "sessions": [
            {"id": r[0], "track": r[1], "car": r[2], "session_type": r[3], "session_date": r[4]}
            for r in rows
        ]
    }
...
def get_tracks_with_laps(request: Request):
    ...
    """).fetchall()
    return {"tracks": [{"trackName": r[0], "bestLapTime": r[1]} for r in rows]}
...
def get_laps(track: str, request: Request):
    ...
    """, (track,)).fetchall()
    return {"laps": [
        {"id": r[0], "lap_number": r[1], "lap_time": r[2], "car": r[3], "session_date": r[4]}
        for r in rows
    ]}
...
def get_telemetry(lap_id: int, request: Request):
    verify_api_key(request)
    rows = con.execute("SELECT * FROM telemetry_samples WHERE lap_id=? ORDER BY timestamp", (lap_id,)).fetchall()
    return {"samples": [
        {"lap_id": r[0], "timestamp": r[1], "speed": r[2], "throttle": r[3], "brake": r[4],
         "steering": r[5], "gear": r[6], "rpm": r[7], "pos_x": r[8], "pos_y": r[9], "pos_z": r[10],
         "gforce_x": r[11], "gforce_y": r[12], "gforce_z": r[13], "yaw": r[14]}
        for r in rows
    ]}
...
def telemetry_status(request: Request):
    verify_api_key(request)
    return {"messages": recorder.messages[-200:]}
